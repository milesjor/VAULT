...
def get_argparse():
    ...
    parser.add_argument('-s', '--save_path', type=str, default="./",
                        help='path/to/per_umi_process (the path to per_umi_process folder generated by VAULT)')
...
def add_pcent(save_path, outvcf):
    vcf = save_path + '/all_snv_from_perfect_umi.vcf'
...
def filter_main(args):
    ...
    outvcf = save_path + '/all_snv_from_perfect_umi.pcent.vcf'
    ...
    cmd = """cat {save_path}/umi_group.flt.summary.txt | {awk} > {save_path}/wrong.group.summary.txt
             ...
             cat {save_path}/all_snv_from_perfect_umi.pcent.vcf | grep "^#" > {save_path}/all_snv_from_perfect_umi.pcent.rem.vcf
             for CHUNK in {save_path}/pattern-file.split.* ; do
                 cat {save_path}/all_snv_from_perfect_umi.pcent.vcf | grep -v "^#" | grep -f "$CHUNK"  >> {save_path}/all_snv_from_perfect_umi.pcent.rem.vcf
             done
             
             rm {save_path}/pattern-file.split.* {save_path}/all.group.lst

             cat {save_path}/all_snv_from_perfect_umi.pcent.rem.vcf | \
                 bcftools filter -s LowQual -e '%QUAL<10 || INFO/DP<3 || INFO/IMF<0.5 || INFO/VARP<{alle_freq} || INFO/ALTC<3' \
                 > {save_path}/all_snv_from_perfect_umi.pcent.rem.flt.vcf

             cat {save_path}/all_snv_from_perfect_umi.pcent.rem.flt.vcf | grep -v LowQual \
                 > {save_path}/pass_snv_from_perfect_umi.flt.vcf
