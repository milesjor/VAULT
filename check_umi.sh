...
if [[ $# -lt 1 ]]; then
    echo "ERROR! This script is a part of VAULT, not intended for sole usage. See commands with 'vault -h'"
    exit
fi
...
# 这里从环境变量获取 cutadapt，如果没设，就用 PATH 里的 cutadapt
CUTADAPT_BIN="${VAULT_CUTADAPT_BIN:-cutadapt}"
...
"${CUTADAPT_BIN}" -e ${error} -j ${thread} -g left_flank=${l} --action=lowercase --discard-untrimmed -o ${s}/${n}.03.adapter_with_left_flank.fasta \
...
"${CUTADAPT_BIN}" -e ${error} -j ${thread} -a right_flank=${r}X --action=lowercase --discard-untrimmed -o ${s}/${n}.05.adapter_with_right_flank.fasta \
